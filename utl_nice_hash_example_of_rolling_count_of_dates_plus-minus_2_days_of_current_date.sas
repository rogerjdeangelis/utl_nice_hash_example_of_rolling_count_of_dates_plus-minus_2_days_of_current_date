...
Your solution will work with data neither sorted nor grouped by ID. That means, however, that the hash table will end up with as many items as there are input records. But if it is intrinsically grouped (as in your sample file), it can be re-coded to reduce the hash memory footprint and limit it to that corresponding to the largest input BY group. In case, a simple iterator will do. The I/O will remain the same (2 passes through the input file).
